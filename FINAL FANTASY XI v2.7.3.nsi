...
Function CopyFilesRecursively
  ...
  loop:
    ...
    file:
      ...
      IfFileExists "$INSTDIR\$R3" 0 copy
        Delete "$INSTDIR\$R3"
      copy:
      CopyFiles /SILENT "$R0\$R2" "$INSTDIR"
    next:
      FindNext $R1 $R2
      Goto loop
  done:
...
Section "XIInstaller" XIInstaller
  ...
  ${If} $FrenchTranslation == "yes"
    DetailPrint "Extracting lang.pak temporarily to $TEMP\lang_temp..."
    IfFileExists "$EXEDIR\archives\lang.pak" 0 lang_missing
      ...
      ${If} $IsUpdate == "yes"
        StrCpy $R0 "$INSTDIR"
      ${Else}
        StrCpy $R0 "$INSTDIR\SquareEnix\FINAL FANTASY XI"
      ${EndIf}
      ${If} ${FileExists} "$TEMP\lang_temp\lang\*.*"
        DetailPrint "Copying French translation files to $R0..."
        CopyFiles "$TEMP\lang_temp\lang\*.*" "$R0"
        RMDir /r "$TEMP\lang_temp\lang"
      ${Else}
        DetailPrint "Copying French translation files to $R0..."
        CopyFiles "$TEMP\lang_temp\*.*" "$R0"
      ${EndIf}
      ...
      ${EndIf}
      StrCpy $TempString "French translation installed/updated in $INSTDIR"
      ${If} $IsUpdate == "no"
        StrCpy $TempString "$TempString\SquareEnix\FINAL FANTASY XI"
      ${EndIf}
      DetailPrint "$TempString"
      Goto lang_done
    lang_missing:
  ...
  ${If} $InstallHDTextures == "yes"
    DetailPrint "Attempting to extract hd_textures.pak to a temporary directory..."
    IfFileExists "$EXEDIR\archives\hd_textures.pak" 0 hd_missing
      ...
      ${Else}
        StrCpy $R0 "$INSTDIR\SquareEnix\FINAL FANTASY XI"
      ${EndIf}
      ${If} ${FileExists} "$TEMP\hd_textures_temp\hd_textures\*.*"
        DetailPrint "Copying HD texture files to $R0..."
        CopyFiles "$TEMP\hd_textures_temp\hd_textures\*.*" "$R0"
        RMDir /r "$TEMP\hd_textures_temp\hd_textures"
      ${Else}
        DetailPrint "Copying HD texture files to $R0..."
        CopyFiles "$TEMP\hd_textures_temp\*.*" "$R0"
      ${EndIf}
      DetailPrint "Cleaning up temporary directory $TEMP\hd_textures_temp..."
      RMDir /r "$TEMP\hd_textures_temp"
      RMDir "$INSTDIR\hd_textures"
      DetailPrint "HD textures installed/updated."
      Goto hd_done
    hd_missing:
  ...
  ${If} $IsUpdate == "no"
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\FINAL FANTASY XI" "UninstallString" "$INSTDIR\SquareEnix\FINAL FANTASY XI\Uninstall FINAL FANTASY XI.exe"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\FINAL FANTASY XI" "QuietUninstallString" "$INSTDIR\SquareEnix\FINAL FANTASY XI\Uninstall FINAL FANTASY XI.exe /S"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\FINAL FANTASY XI" "InstallLocation" "$INSTDIR"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\FINAL FANTASY XI" "DisplayIcon" "$INSTDIR\SquareEnix\FINAL FANTASY XI\installer.ico"
    ...
    ${If} $IsUpdate == "yes"
      RegDLL "$INSTDIR\FFXi.dll"
      RegDLL "$INSTDIR\FFXiMain.dll"
      RegDLL "$INSTDIR\FFXiResource.dll"
      RegDLL "$INSTDIR\FFXiVersions.dll"
    ${Else}
    ...
    ${Else}
      WriteUninstaller "$INSTDIR\SquareEnix\FINAL FANTASY XI\Uninstall FINAL FANTASY XI.exe"
    ${EndIf}
    ...
    ${Else}
      SetOutPath "$INSTDIR\Ashita"
      CreateShortcut "$DESKTOP\Play FINAL FANTASY XI.lnk" "$INSTDIR\Ashita\Ashita.exe" "" "$INSTDIR\SquareEnix\FINAL FANTASY XI\installer.ico"
      CreateDirectory "$SMPROGRAMS\FINAL FANTASY XI"
      CreateShortcut "$SMPROGRAMS\FINAL FANTASY XI\Play FINAL FANTASY XI.lnk" "$INSTDIR\Ashita\Ashita.exe" "" "$INSTDIR\SquareEnix\FINAL FANTASY XI\installer.ico"
      CreateShortcut "$SMPROGRAMS\FINAL FANTASY XI\Uninstall FINAL FANTASY XI.lnk" "$INSTDIR\SquareEnix\FINAL FANTASY XI\Uninstall FINAL FANTASY XI.exe"
    ${EndIf}
  ${EndIf}
SectionEnd
...
Section "Uninstall"
  SetOutPath "$INSTDIR"
  ${If} $IsUpdate == "yes"
    UnRegDLL "$INSTDIR\FFXi.dll"
    UnRegDLL "$INSTDIR\FFXiMain.dll"
    UnRegDLL "$INSTDIR\FFXiResource.dll"
    UnRegDLL "$INSTDIR\FFXiVersions.dll"
  ${Else}
  ...
  ${If} $IsUpdate == "yes"
    RMDir /r "$INSTDIR"
  ${Else}
    RMDir /r "$INSTDIR\SquareEnix\FINAL FANTASY XI"
  ${EndIf}
  ...
  ${Else}
    Delete "$INSTDIR\SquareEnix\FINAL FANTASY XI\installer.ico"
    Delete "$INSTDIR\SquareEnix\FINAL FANTASY XI\Uninstall FINAL FANTASY XI.exe"
  ${EndIf}
