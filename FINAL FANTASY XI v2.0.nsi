...
;Written by Joost Verburg, Modified by Fox_Mulder.
...
;Variables globales
  ...
  Var FrenchTranslation ; Variable pour stocker le choix de traduction en français
...
Function TranslationPage
  !insertmacro MUI_HEADER_TEXT "Options de traduction" "Choisissez vos préférences de langue."
  ...
  ${NSD_CreateCheckbox} 0 0u 100% 10u "Installer la traduction en français"
  Pop $FrenchTranslation
  ${NSD_Check} $FrenchTranslation ; Cochée par défaut (retirez cette ligne si vous voulez décochée)
...
Section "XIInstaller" XIInstaller
  ...
  ; Étape 1 : Décompresser data.pak dans $INSTDIR
  DetailPrint "Extraction des fichiers du jeu depuis data.pak..."
  File /oname=$TEMP\data.pak "data.pak" ; Copie data.pak dans $TEMP
  Nsis7z::ExtractWithDetails "$TEMP\data.pak" "Installation des fichiers du jeu %s..."
  Delete "$TEMP\data.pak" ; Nettoie le fichier temporaire

  ; Étape 2 : Si traduction cochée, décompresser lang.pak dans le même répertoire
  ${If} $FrenchTranslation == "yes"
    DetailPrint "Extraction des traductions en français depuis lang.pak..."
    File /oname=$TEMP\lang.pak "lang.pak" ; Copie lang.pak dans $TEMP
    Nsis7z::ExtractWithDetails "$TEMP\lang.pak" "Installation de la traduction française %s..."
    Delete "$TEMP\lang.pak" ; Nettoie le fichier temporaire
    ; Optionnel : ajuster une clé de registre pour indiquer la langue française
